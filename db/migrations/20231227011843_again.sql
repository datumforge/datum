...
CREATE TABLE `new_email_verification_tokens` (`id` text NOT NULL, `created_at` datetime NOT NULL, `updated_at` datetime NOT NULL, `created_by` text NULL, `updated_by` text NULL, `deleted_at` datetime NULL, `deleted_by` text NULL, `token` text NULL, `ttl` datetime NULL, `email` text NULL, `secret` text NULL, `user_email_verification_tokens` text NOT NULL, `user_children` text NULL, PRIMARY KEY (`id`), CONSTRAINT `email_verification_tokens_users_email_verification_tokens` FOREIGN KEY (`user_email_verification_tokens`) REFERENCES `users` (`id`) ON DELETE NO ACTION, CONSTRAINT `email_verification_tokens_users_children` FOREIGN KEY (`user_children`) REFERENCES `users` (`id`) ON DELETE SET NULL);
