...
import (
	...
	entUser "github.com/datumforge/datum/ent/User"
	"github.com/rs/zerolog/log"

	"github.com/labstack/echo/v4"

	"github.com/datumforge/datum/ent"
)

// CustomContext contains the echo.Context and request context.Context
type UserCustomContext struct {
	echo.Context
	ctx  context.Context
	User *ent.User
}
...
func GetUserMiddleware(client *ent.Client) echo.MiddlewareFunc {
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			...
			if authMethod == "local" {
				...
				user, err := client.User.Query().Where(entUser.ID(id)).Only(c.Request().Context())
				if err != nil {
					return echo.NewHTTPError(http.StatusUnauthorized, "Invalid access token")
				}
				cc := &CustomContext{
