...
func (m *IntegrationMutation) CheckAccessForEdit(ctx context.Context) error {
	...
	// check if the input is a CreateIntegrationInput
	input, ok := gInput.(CreateIntegrationInput)
	if ok {
...
func (m *InviteMutation) CheckAccessForEdit(ctx context.Context) error {
	...
	// check if the input is a CreateInviteInput
	input, ok := gInput.(CreateInviteInput)
	if ok {
		ac.ObjectID = input.OwnerID
...
func (q *TierQuery) CheckAccess(ctx context.Context) error {
	gCtx := graphql.GetFieldContext(ctx)
	...
	if gCtx != nil {
		...
		whereArg := gCtx.Args["where"]
		if whereArg != nil {
			where, ok := whereArg.(*TierWhereInput)
			if ok && where != nil && where.OwnerID != nil {
				ac.ObjectID = *where.OwnerID
			}
		}
...
func (m *TierMutation) CheckAccessForEdit(ctx context.Context) error {
	ac := fgax.AccessCheck{
		Relation:   fgax.CanEdit,
		ObjectType: "organization",
	}
	...
	// check if the input is a CreateTierInput
	input, ok := gInput.(CreateTierInput)
	if ok {
		ac.ObjectID = *input.OwnerID
	...
	if ac.ObjectID == "" && "id" != "ownerid" {
		...
		if ok {
			// allow this query to run
			reqCtx := privacy.DecisionContext(ctx, privacy.Allow)
			ob, err := m.Client().Tier.Query().Where(tier.ID(id)).Only(reqCtx)
			if err != nil {
				return privacy.Allowf("nil request, bypassing auth check")
			}
...
func (m *TierMutation) CheckAccessForDelete(ctx context.Context) error {
	ac := fgax.AccessCheck{
		Relation:   fgax.CanDelete,
		ObjectType: "organization",
	}
