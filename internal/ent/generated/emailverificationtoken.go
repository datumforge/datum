...
type EmailVerificationToken struct {
	...
	// the ttl of the verification token which needs to be explictly set, otherwise defaults to time.Now
...
func (*EmailVerificationToken) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case emailverificationtoken.ForeignKeys[0]: // user_email_verification_tokens
			values[i] = new(sql.NullString)
		case emailverificationtoken.ForeignKeys[1]: // user_children
			values[i] = new(sql.NullString)
		default:
...
func (evt *EmailVerificationToken) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case emailverificationtoken.ForeignKeys[1]:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field user_children", values[i])
			} else if value.Valid {
				evt.user_children = new(string)
				*evt.user_children = value.String
			}
		default:
...
func (evt *EmailVerificationToken) String() string {
	...
	builder.WriteString(evt.TTL.Format(time.ANSIC))
	builder.WriteString(", ")
