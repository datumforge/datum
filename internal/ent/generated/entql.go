...
var schemaGraph = func() *sqlgraph.Schema {
	...
	graph.Nodes[16] = &sqlgraph.Node{
		...
		Fields: map[string]*sqlgraph.FieldSpec{
			...
			template.FieldOtherconfig: {Type: field.TypeOther, Column: template.FieldOtherconfig},
			template.FieldPair:        {Type: field.TypeBytes, Column: template.FieldPair},
			template.FieldURL:         {Type: field.TypeString, Column: template.FieldURL},
		},
	}
...
// WhereOtherconfig applies the entql other predicate on the otherconfig field.
func (f *TemplateFilter) WhereOtherconfig(p entql.OtherP) {
	f.Where(p.Field(template.FieldOtherconfig))
}

// WherePair applies the entql []byte predicate on the pair field.
func (f *TemplateFilter) WherePair(p entql.BytesP) {
	f.Where(p.Field(template.FieldPair))
}

// WhereURL applies the entql string predicate on the url field.
func (f *TemplateFilter) WhereURL(p entql.StringP) {
	f.Where(p.Field(template.FieldURL))
}
