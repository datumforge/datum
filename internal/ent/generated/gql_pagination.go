...
// SubscriptionEdge is the edge representation of Subscription.
type SubscriptionEdge struct {
	Node   *Subscription `json:"node"`
	Cursor Cursor        `json:"cursor"`
}

// SubscriptionConnection is the connection containing edges to Subscription.
...
func (c *SubscriptionConnection) build(nodes []*Subscription, pager *subscriptionPager, after *Cursor, first *int, before *Cursor, last *int) {
	c.PageInfo.HasNextPage = before != nil
	c.PageInfo.HasPreviousPage = after != nil
	if first != nil && *first+1 == len(nodes) {
		c.PageInfo.HasNextPage = true
	...
	var nodeAt func(int) *Subscription
	if last != nil {
		n := len(nodes) - 1
		nodeAt = func(i int) *Subscription {
			return nodes[n-i]
		}
	} else {
		nodeAt = func(i int) *Subscription {
	...
	c.Edges = make([]*SubscriptionEdge, len(nodes))
	for i := range nodes {
		node := nodeAt(i)
		c.Edges[i] = &SubscriptionEdge{
			Node:   node,
			Cursor: pager.toCursor(node),
		}
	}
...
// SubscriptionPaginateOption enables pagination customization.
type SubscriptionPaginateOption func(*subscriptionPager) error

// WithSubscriptionOrder configures pagination ordering.
func WithSubscriptionOrder(order *SubscriptionOrder) SubscriptionPaginateOption {
	if order == nil {
		order = DefaultSubscriptionOrder
	}
	o := *order
	return func(pager *subscriptionPager) error {
		if err := o.Direction.Validate(); err != nil {
			return err
		}
		if o.Field == nil {
			o.Field = DefaultSubscriptionOrder.Field
		}
		pager.order = &o
		return nil
	}
...
// WithSubscriptionFilter configures pagination filter.
func WithSubscriptionFilter(filter func(*SubscriptionQuery) (*SubscriptionQuery, error)) SubscriptionPaginateOption {
	return func(pager *subscriptionPager) error {
		if filter == nil {
...
func newSubscriptionPager(opts []SubscriptionPaginateOption, reverse bool) (*subscriptionPager, error) {
	pager := &subscriptionPager{reverse: reverse}
	for _, opt := range opts {
		if err := opt(pager); err != nil {
			return nil, err
		}
	...
	if pager.order == nil {
		pager.order = DefaultSubscriptionOrder
	}
	return pager, nil
}

func (p *subscriptionPager) applyFilter(query *SubscriptionQuery) (*SubscriptionQuery, error) {
	if p.filter != nil {
		return p.filter(query)
	}
	return query, nil
...
func (p *subscriptionPager) toCursor(s *Subscription) Cursor {
	return p.order.Field.toCursor(s)
}

func (p *subscriptionPager) applyCursors(query *SubscriptionQuery, after, before *Cursor) (*SubscriptionQuery, error) {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	for _, predicate := range entgql.CursorsPredicate(after, before, DefaultSubscriptionOrder.Field.column, p.order.Field.column, direction) {
		query = query.Where(predicate)
	}
	return query, nil
}

func (p *subscriptionPager) applyOrder(query *SubscriptionQuery) *SubscriptionQuery {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	query = query.Order(p.order.Field.toTerm(direction.OrderTermOption()))
	if p.order.Field != DefaultSubscriptionOrder.Field {
		query = query.Order(DefaultSubscriptionOrder.Field.toTerm(direction.OrderTermOption()))
	}
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(p.order.Field.column)
	}
...
func (p *subscriptionPager) orderExpr(query *SubscriptionQuery) sql.Querier {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	...
	return sql.ExprFunc(func(b *sql.Builder) {
		b.Ident(p.order.Field.column).Pad().WriteString(string(direction))
		if p.order.Field != DefaultSubscriptionOrder.Field {
			b.Comma().Ident(DefaultSubscriptionOrder.Field.column).Pad().WriteString(string(direction))
		}
	})
}

// Paginate executes the query and returns a relay based cursor connection to Subscription.
func (s *SubscriptionQuery) Paginate(
	ctx context.Context, after *Cursor, first *int,
	before *Cursor, last *int, opts ...SubscriptionPaginateOption,
) (*SubscriptionConnection, error) {
	if err := validateFirstLast(first, last); err != nil {
		return nil, err
	}
	pager, err := newSubscriptionPager(opts, last != nil)
	if err != nil {
		return nil, err
	}
	if s, err = pager.applyFilter(s); err != nil {
	...
	conn := &SubscriptionConnection{Edges: []*SubscriptionEdge{}}
	ignoredEdges := !hasCollectedField(ctx, edgesField)
	if hasCollectedField(ctx, totalCountField) || hasCollectedField(ctx, pageInfoField) {
		hasPagination := after != nil || first != nil || before != nil || last != nil
		if hasPagination || ignoredEdges {
...
// SubscriptionOrderField defines the ordering field of Subscription.
type SubscriptionOrderField struct {
	// Value extracts the ordering value from the given Subscription.
	Value    func(*Subscription) (ent.Value, error)
	column   string // field or computed.
	toTerm   func(...sql.OrderTermOption) subscription.OrderOption
	toCursor func(*Subscription) Cursor
}

// SubscriptionOrder defines the ordering of Subscription.
type SubscriptionOrder struct {
	Direction OrderDirection          `json:"direction"`
	Field     *SubscriptionOrderField `json:"field"`
}

// DefaultSubscriptionOrder is the default ordering of Subscription.
var DefaultSubscriptionOrder = &SubscriptionOrder{
	Direction: entgql.OrderDirectionAsc,
	Field: &SubscriptionOrderField{
		Value: func(s *Subscription) (ent.Value, error) {
			return s.ID, nil
		},
		column: subscription.FieldID,
		toTerm: subscription.ByID,
		toCursor: func(s *Subscription) Cursor {
			return Cursor{ID: s.ID}
		},
	},
}

// ToEdge converts Subscription into SubscriptionEdge.
func (s *Subscription) ToEdge(order *SubscriptionOrder) *SubscriptionEdge {
	if order == nil {
		order = DefaultSubscriptionOrder
	}
