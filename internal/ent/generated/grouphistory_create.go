...
func (ghc *GroupHistoryCreate) check() error {
	...
	if _, ok := ghc.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "created_at", err: errors.New(`generated: missing required field "GroupHistory.created_at"`)}
	}
	if _, ok := ghc.mutation.UpdatedAt(); !ok {
		return &ValidationError{Name: "updated_at", err: errors.New(`generated: missing required field "GroupHistory.updated_at"`)}
	}
	...
	if v, ok := ghc.mutation.Name(); ok {
		if err := grouphistory.NameValidator(v); err != nil {
			return &ValidationError{Name: "name", err: fmt.Errorf(`generated: validator failed for field "GroupHistory.name": %w`, err)}
		}
	}
	if _, ok := ghc.mutation.DisplayName(); !ok {
		return &ValidationError{Name: "display_name", err: errors.New(`generated: missing required field "GroupHistory.display_name"`)}
	}
	if v, ok := ghc.mutation.DisplayName(); ok {
		if err := grouphistory.DisplayNameValidator(v); err != nil {
			return &ValidationError{Name: "display_name", err: fmt.Errorf(`generated: validator failed for field "GroupHistory.display_name": %w`, err)}
		}
	}
	return nil
}
