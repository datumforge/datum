...
func (ghu *GroupHistoryUpdate) defaults() {
	if _, ok := ghu.mutation.UpdatedAt(); !ok {
...
func (ghu *GroupHistoryUpdate) check() error {
	if v, ok := ghu.mutation.Name(); ok {
	...
	if v, ok := ghu.mutation.DisplayName(); ok {
...
func (ghuo *GroupHistoryUpdateOne) defaults() {
	if _, ok := ghuo.mutation.UpdatedAt(); !ok {
		v := grouphistory.UpdateDefaultUpdatedAt()
...
// check runs all checks and user-defined validators on the builder.
func (ghuo *GroupHistoryUpdateOne) check() error {
	if v, ok := ghuo.mutation.Name(); ok {
		if err := grouphistory.NameValidator(v); err != nil {
			return &ValidationError{Name: "name", err: fmt.Errorf(`generated: validator failed for field "GroupHistory.name": %w`, err)}
		}
	}
	if v, ok := ghuo.mutation.DisplayName(); ok {
		if err := grouphistory.DisplayNameValidator(v); err != nil {
			return &ValidationError{Name: "display_name", err: fmt.Errorf(`generated: validator failed for field "GroupHistory.display_name": %w`, err)}
		}
	}
