...
type GroupMembership struct {
	...
	// GroupRole holds the value of the "group_role" field.
	GroupRole enums.Role `json:"group_role,omitempty"`
...
// was not loaded in eager-loading, or loaded but was not found.
func (e GroupMembershipEdges) RoleOrErr() (*Role, error) {
	if e.loadedTypes[2] {
		if e.Role == nil {
			// Edge was loaded but was not found.
...
func (*GroupMembership) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case groupmembership.FieldID, groupmembership.FieldCreatedBy, groupmembership.FieldUpdatedBy, groupmembership.FieldGroupRole:
		...
		case groupmembership.ForeignKeys[0]: // group_members
			values[i] = new(sql.NullString)
		case groupmembership.ForeignKeys[1]: // role_id
			values[i] = new(sql.NullString)
		case groupmembership.ForeignKeys[2]: // user_group_memberships
			values[i] = new(sql.NullString)
		default:
	...
	for i := range columns {
		...
		case groupmembership.FieldGroupRole:
			if value, ok := values[i].(*sql.NullString); !ok {
				...
				gm.GroupRole = enums.Role(value.String)
			}
		case groupmembership.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field group_members", values[i])
			} else if value.Valid {
				gm.group_members = new(string)
				*gm.group_members = value.String
			}
		case groupmembership.ForeignKeys[1]:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field role_id", values[i])
			} else if value.Valid {
				gm.role_id = new(string)
				*gm.role_id = value.String
			}
		case groupmembership.ForeignKeys[2]:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field user_group_memberships", values[i])
			} else if value.Valid {
				gm.user_group_memberships = new(string)
				*gm.user_group_memberships = value.String
			}
		default:
...
// QueryRole queries the "role" edge of the GroupMembership entity.
func (gm *GroupMembership) QueryRole() *RoleQuery {
	return NewGroupMembershipClient(gm.config).QueryRole(gm)
}
...
func (gm *GroupMembership) String() string {
	...
	builder.WriteString(fmt.Sprintf("%v", gm.GroupRole))
	builder.WriteByte(')')
