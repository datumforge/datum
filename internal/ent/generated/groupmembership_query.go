...
func (gmq *GroupMembershipQuery) QueryRole() *RoleQuery {
	query := (&RoleClient{config: gmq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := gmq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (gmq *GroupMembershipQuery) WithRole(opts ...func(*RoleQuery)) *GroupMembershipQuery {
	query := (&RoleClient{config: gmq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (gmq *GroupMembershipQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*GroupMembership, error) {
	...
	if gmq.withGroup != nil || gmq.withUser != nil || gmq.withRole != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := gmq.withRole; query != nil {
		if err := gmq.loadRole(ctx, query, nodes, nil,
			func(n *GroupMembership, e *Role) { n.Edges.Role = e }); err != nil {
			return nil, err
		}
	}
...
func (gmq *GroupMembershipQuery) loadRole(ctx context.Context, query *RoleQuery, nodes []*GroupMembership, init func(*GroupMembership), assign func(*GroupMembership, *Role)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*GroupMembership)
	for i := range nodes {
