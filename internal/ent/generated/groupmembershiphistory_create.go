...
func (gmhc *GroupMembershipHistoryCreate) check() error {
	...
	if _, ok := gmhc.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "created_at", err: errors.New(`generated: missing required field "GroupMembershipHistory.created_at"`)}
	}
	if _, ok := gmhc.mutation.UpdatedAt(); !ok {
		return &ValidationError{Name: "updated_at", err: errors.New(`generated: missing required field "GroupMembershipHistory.updated_at"`)}
	}
	if _, ok := gmhc.mutation.Role(); !ok {
