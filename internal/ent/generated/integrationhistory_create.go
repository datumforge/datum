...
func (ihc *IntegrationHistoryCreate) check() error {
	...
	if _, ok := ihc.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "created_at", err: errors.New(`generated: missing required field "IntegrationHistory.created_at"`)}
	}
	if _, ok := ihc.mutation.UpdatedAt(); !ok {
		return &ValidationError{Name: "updated_at", err: errors.New(`generated: missing required field "IntegrationHistory.updated_at"`)}
	}
	...
	if v, ok := ihc.mutation.Name(); ok {
		if err := integrationhistory.NameValidator(v); err != nil {
			return &ValidationError{Name: "name", err: fmt.Errorf(`generated: validator failed for field "IntegrationHistory.name": %w`, err)}
		}
	}
	if _, ok := ihc.mutation.SecretName(); !ok {
		return &ValidationError{Name: "secret_name", err: errors.New(`generated: missing required field "IntegrationHistory.secret_name"`)}
	}
	return nil
}
