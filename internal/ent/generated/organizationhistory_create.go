...
func (ohc *OrganizationHistoryCreate) check() error {
	...
	if _, ok := ohc.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "created_at", err: errors.New(`generated: missing required field "OrganizationHistory.created_at"`)}
	}
	if _, ok := ohc.mutation.UpdatedAt(); !ok {
		return &ValidationError{Name: "updated_at", err: errors.New(`generated: missing required field "OrganizationHistory.updated_at"`)}
	}
	...
	if v, ok := ohc.mutation.Name(); ok {
		if err := organizationhistory.NameValidator(v); err != nil {
			return &ValidationError{Name: "name", err: fmt.Errorf(`generated: validator failed for field "OrganizationHistory.name": %w`, err)}
		}
	}
	if _, ok := ohc.mutation.DisplayName(); !ok {
	...
	if v, ok := ohc.mutation.DisplayName(); ok {
		if err := organizationhistory.DisplayNameValidator(v); err != nil {
			return &ValidationError{Name: "display_name", err: fmt.Errorf(`generated: validator failed for field "OrganizationHistory.display_name": %w`, err)}
		}
	}
	if _, ok := ohc.mutation.PersonalOrg(); !ok {
		return &ValidationError{Name: "personal_org", err: errors.New(`generated: missing required field "OrganizationHistory.personal_org"`)}
	}
	return nil
}
