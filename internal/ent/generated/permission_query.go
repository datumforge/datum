...
func (pq *PermissionQuery) QueryRolePermission() *RolePermissionQuery {
...
func (pq *PermissionQuery) Clone() *PermissionQuery {
	...
	return &PermissionQuery{
		...
		withRolePermission: pq.withRolePermission.Clone(),
...
func (pq *PermissionQuery) WithRolePermission(opts ...func(*RolePermissionQuery)) *PermissionQuery {
	query := (&RolePermissionClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withRolePermission = query
	return pq
}
...
func (pq *PermissionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Permission, error) {
	...
	if query := pq.withRolePermission; query != nil {
		if err := pq.loadRolePermission(ctx, query, nodes,
	...
	for name, query := range pq.withNamedRolePermission {
		if err := pq.loadRolePermission(ctx, query, nodes,
			func(n *Permission) { n.appendNamedRolePermission(name) },
			func(n *Permission, e *RolePermission) { n.appendNamedRolePermission(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (pq *PermissionQuery) loadRoles(ctx context.Context, query *RoleQuery, nodes []*Permission, init func(*Permission), assign func(*Permission, *Role)) error {
	...
	query.Where(func(s *sql.Selector) {
		...
		joinT.Schema(pq.schemaConfig.RolePermission)
...
func (pq *PermissionQuery) loadRolePermission(ctx context.Context, query *RolePermissionQuery, nodes []*Permission, init func(*Permission), assign func(*Permission, *RolePermission)) error {
...
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (pq *PermissionQuery) WithNamedRolePermission(name string, opts ...func(*RolePermissionQuery)) *PermissionQuery {
	query := (&RolePermissionClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if pq.withNamedRolePermission == nil {
		pq.withNamedRolePermission = make(map[string]*RolePermissionQuery)
	}
	pq.withNamedRolePermission[name] = query
	return pq
}
