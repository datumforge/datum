...
type RoleEdges struct {
	...
	GroupRoles []*GroupMembership `json:"group_roles,omitempty"`
	...
	namedGroupRoles  map[string][]*GroupMembership
...
// was not loaded in eager-loading.
func (e RoleEdges) GroupRolesOrErr() ([]*GroupMembership, error) {
	if e.loadedTypes[1] {
...
func (*Role) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case role.FieldID, role.FieldCreatedBy, role.FieldUpdatedBy, role.FieldName:
...
// QueryGroupRoles queries the "group_roles" edge of the Role entity.
func (r *Role) QueryGroupRoles() *GroupMembershipQuery {
	return NewRoleClient(r.config).QueryGroupRoles(r)
}
...
// NamedGroupRoles returns the GroupRoles named value or an error if the edge was not
// loaded in eager-loading with this name.
func (r *Role) NamedGroupRoles(name string) ([]*GroupMembership, error) {
	if r.Edges.namedGroupRoles == nil {
		return nil, &NotLoadedError{edge: name}
	}
	nodes, ok := r.Edges.namedGroupRoles[name]
	if !ok {
		return nil, &NotLoadedError{edge: name}
	}
	return nodes, nil
...
func (r *Role) appendNamedGroupRoles(name string, edges ...*GroupMembership) {
	if r.Edges.namedGroupRoles == nil {
		r.Edges.namedGroupRoles = make(map[string][]*GroupMembership)
	}
	if len(edges) == 0 {
		r.Edges.namedGroupRoles[name] = []*GroupMembership{}
	} else {
		r.Edges.namedGroupRoles[name] = append(r.Edges.namedGroupRoles[name], edges...)
	}
}
