...
func (r *Role) NamedUser(name string) ([]*User, error) {
...
func (r *Role) appendNamedUser(name string, edges ...*User) {
	if r.Edges.namedUser == nil {
...
// loaded in eager-loading with this name.
func (r *Role) NamedRolesUsers(name string) ([]*RoleUser, error) {
	if r.Edges.namedRolesUsers == nil {
		return nil, &NotLoadedError{edge: name}
	}
	nodes, ok := r.Edges.namedRolesUsers[name]
	if !ok {
		return nil, &NotLoadedError{edge: name}
	}
	return nodes, nil
...
func (r *Role) appendNamedRolesUsers(name string, edges ...*RoleUser) {
	if r.Edges.namedRolesUsers == nil {
		r.Edges.namedRolesUsers = make(map[string][]*RoleUser)
	}
	if len(edges) == 0 {
		r.Edges.namedRolesUsers[name] = []*RoleUser{}
	} else {
		r.Edges.namedRolesUsers[name] = append(r.Edges.namedRolesUsers[name], edges...)
	}
}
