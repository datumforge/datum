...
type RoleQuery struct {
	...
	withNamedRolePermission map[string]*RolePermissionQuery
	withNamedUserRole       map[string]*UserRoleQuery
...
func (rq *RoleQuery) QueryUserRole() *UserRoleQuery {
...
func (rq *RoleQuery) WithUserRole(opts ...func(*UserRoleQuery)) *RoleQuery {
	query := (&UserRoleClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	rq.withUserRole = query
	return rq
}
...
func (rq *RoleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Role, error) {
	...
	if query := rq.withRolePermission; query != nil {
		if err := rq.loadRolePermission(ctx, query, nodes,
	...
	if query := rq.withUserRole; query != nil {
		if err := rq.loadUserRole(ctx, query, nodes,
	...
	for name, query := range rq.withNamedRolePermission {
		if err := rq.loadRolePermission(ctx, query, nodes,
			func(n *Role) { n.appendNamedRolePermission(name) },
			func(n *Role, e *RolePermission) { n.appendNamedRolePermission(name, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range rq.withNamedUserRole {
		if err := rq.loadUserRole(ctx, query, nodes,
			func(n *Role) { n.appendNamedUserRole(name) },
			func(n *Role, e *UserRole) { n.appendNamedUserRole(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (rq *RoleQuery) loadPermissions(ctx context.Context, query *PermissionQuery, nodes []*Role, init func(*Role), assign func(*Role, *Permission)) error {
	...
	query.Where(func(s *sql.Selector) {
		...
		joinT.Schema(rq.schemaConfig.RolePermission)
...
func (rq *RoleQuery) loadUsers(ctx context.Context, query *UserQuery, nodes []*Role, init func(*Role), assign func(*Role, *User)) error {
	...
	query.Where(func(s *sql.Selector) {
		...
		joinT.Schema(rq.schemaConfig.UserRole)
...
func (rq *RoleQuery) loadRolePermission(ctx context.Context, query *RolePermissionQuery, nodes []*Role, init func(*Role), assign func(*Role, *RolePermission)) error {
...
func (rq *RoleQuery) loadUserRole(ctx context.Context, query *UserRoleQuery, nodes []*Role, init func(*Role), assign func(*Role, *UserRole)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Role)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
...
// WithNamedRolePermission tells the query-builder to eager-load the nodes that are connected to the "role_permission"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (rq *RoleQuery) WithNamedRolePermission(name string, opts ...func(*RolePermissionQuery)) *RoleQuery {
	query := (&RolePermissionClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if rq.withNamedRolePermission == nil {
		rq.withNamedRolePermission = make(map[string]*RolePermissionQuery)
	}
	rq.withNamedRolePermission[name] = query
	return rq
}

// WithNamedUserRole tells the query-builder to eager-load the nodes that are connected to the "user_role"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (rq *RoleQuery) WithNamedUserRole(name string, opts ...func(*UserRoleQuery)) *RoleQuery {
	query := (&UserRoleClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if rq.withNamedUserRole == nil {
		rq.withNamedUserRole = make(map[string]*UserRoleQuery)
	}
	rq.withNamedUserRole[name] = query
	return rq
}
