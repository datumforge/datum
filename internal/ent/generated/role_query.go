...
type RoleQuery struct {
	...
	withUsers            *UserQuery
	withGroupRoles       *GroupMembershipQuery
	withPermissions      *PermissionQuery
	modifiers            []func(*sql.Selector)
	loadTotal            []func(context.Context, []*Role) error
	withNamedUsers       map[string]*UserQuery
	withNamedGroupRoles  map[string]*GroupMembershipQuery
...
func (rq *RoleQuery) QueryGroupRoles() *GroupMembershipQuery {
	query := (&GroupMembershipClient{config: rq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
		}
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, role.GroupRolesTable, role.GroupRolesColumn),
		)
		schemaConfig := rq.schemaConfig
...
func (rq *RoleQuery) Clone() *RoleQuery {
	...
	return &RoleQuery{
		...
		withGroupRoles:  rq.withGroupRoles.Clone(),
...
// WithGroupRoles tells the query-builder to eager-load the nodes that are connected to
// the "group_roles" edge. The optional arguments are used to configure the query builder of the edge.
func (rq *RoleQuery) WithGroupRoles(opts ...func(*GroupMembershipQuery)) *RoleQuery {
	query := (&GroupMembershipClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	rq.withGroupRoles = query
	return rq
}
...
func (rq *RoleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Role, error) {
	...
	if query := rq.withGroupRoles; query != nil {
		if err := rq.loadGroupRoles(ctx, query, nodes,
			func(n *Role) { n.Edges.GroupRoles = []*GroupMembership{} },
			func(n *Role, e *GroupMembership) { n.Edges.GroupRoles = append(n.Edges.GroupRoles, e) }); err != nil {
			return nil, err
		}
	}
	...
	for name, query := range rq.withNamedGroupRoles {
		if err := rq.loadGroupRoles(ctx, query, nodes,
			func(n *Role) { n.appendNamedGroupRoles(name) },
			func(n *Role, e *GroupMembership) { n.appendNamedGroupRoles(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (rq *RoleQuery) loadGroupRoles(ctx context.Context, query *GroupMembershipQuery, nodes []*Role, init func(*Role), assign func(*Role, *GroupMembership)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Role)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.GroupMembership(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(role.GroupRolesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
// WithNamedGroupRoles tells the query-builder to eager-load the nodes that are connected to the "group_roles"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (rq *RoleQuery) WithNamedGroupRoles(name string, opts ...func(*GroupMembershipQuery)) *RoleQuery {
	query := (&GroupMembershipClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if rq.withNamedGroupRoles == nil {
		rq.withNamedGroupRoles = make(map[string]*GroupMembershipQuery)
	}
	rq.withNamedGroupRoles[name] = query
	return rq
}
