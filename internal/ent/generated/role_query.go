...
func (rq *RoleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Role, error) {
	...
	for name, query := range rq.withNamedRolesUsers {
		if err := rq.loadRolesUsers(ctx, query, nodes,
			func(n *Role) { n.appendNamedRolesUsers(name) },
			func(n *Role, e *RoleUser) { n.appendNamedRolesUsers(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (rq *RoleQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Role, init func(*Role), assign func(*Role, *User)) error {
	...
	query.Where(func(s *sql.Selector) {
		...
		joinT.Schema(rq.schemaConfig.RoleUser)
...
func (rq *RoleQuery) WithNamedUser(name string, opts ...func(*UserQuery)) *RoleQuery {
	query := (&UserClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (rq *RoleQuery) WithNamedRolesUsers(name string, opts ...func(*RoleUserQuery)) *RoleQuery {
	query := (&RoleUserClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if rq.withNamedRolesUsers == nil {
		rq.withNamedRolesUsers = make(map[string]*RoleUserQuery)
	}
	rq.withNamedRolesUsers[name] = query
	return rq
}
