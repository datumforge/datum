...
func (*Template) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case template.FieldPair:
			values[i] = new(customtypes.Pair)
		case template.FieldOtherconfig:
			values[i] = new(schematype.TemplateConfig)
		...
		case template.FieldURL:
			values[i] = template.ValueScanner.URL.ScanValue()
		default:
...
func (t *Template) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case template.FieldOtherconfig:
			if value, ok := values[i].(*schematype.TemplateConfig); !ok {
				return fmt.Errorf("unexpected type %T for field otherconfig", values[i])
			} else if value != nil {
				t.Otherconfig = value
			}
		case template.FieldPair:
			if value, ok := values[i].(*customtypes.Pair); !ok {
				return fmt.Errorf("unexpected type %T for field pair", values[i])
			} else if value != nil {
				t.Pair = *value
			}
		case template.FieldURL:
			if value, err := template.ValueScanner.URL.FromValue(values[i]); err != nil {
				return err
			} else {
				t.URL = value
			}
...
func (t *Template) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("otherconfig=")
	builder.WriteString(fmt.Sprintf("%v", t.Otherconfig))
	builder.WriteString(", ")
	builder.WriteString("pair=")
	builder.WriteString(fmt.Sprintf("%v", t.Pair))
	builder.WriteString(", ")
