...
func (tc *TemplateCreate) SetOtherconfig(sc *schematype.TemplateConfig) *TemplateCreate {
	tc.mutation.SetOtherconfig(sc)
	return tc
}

// SetPair sets the "pair" field.
func (tc *TemplateCreate) SetPair(c customtypes.Pair) *TemplateCreate {
	tc.mutation.SetPair(c)
	return tc
}

// SetNillablePair sets the "pair" field if the given value is not nil.
func (tc *TemplateCreate) SetNillablePair(c *customtypes.Pair) *TemplateCreate {
	if c != nil {
		tc.SetPair(*c)
	}
	return tc
...
func (tc *TemplateCreate) SetURL(u *url.URL) *TemplateCreate {
...
func (tc *TemplateCreate) defaults() error {
	...
	if _, ok := tc.mutation.Pair(); !ok {
		if template.DefaultPair == nil {
			return fmt.Errorf("generated: uninitialized template.DefaultPair (forgotten import generated/runtime?)")
		}
		v := template.DefaultPair()
		tc.mutation.SetPair(v)
	}
...
func (tc *TemplateCreate) check() error {
	...
	if _, ok := tc.mutation.Pair(); !ok {
		return &ValidationError{Name: "pair", err: errors.New(`generated: missing required field "Template.pair"`)}
	}
	if _, ok := tc.mutation.URL(); !ok {
		return &ValidationError{Name: "url", err: errors.New(`generated: missing required field "Template.url"`)}
	}
	if _, ok := tc.mutation.OwnerID(); !ok {
...
func (tc *TemplateCreate) createSpec() (*Template, *sqlgraph.CreateSpec, error) {
	var (
	...
	if value, ok := tc.mutation.Otherconfig(); ok {
		_spec.SetField(template.FieldOtherconfig, field.TypeOther, value)
		_node.Otherconfig = value
	}
	if value, ok := tc.mutation.Pair(); ok {
