...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	for name, query := range uq.withNamedUserRole {
		if err := uq.loadUserRole(ctx, query, nodes,
			func(n *User) { n.appendNamedUserRole(name) },
			func(n *User, e *UserRole) { n.appendNamedUserRole(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (uq *UserQuery) loadRoles(ctx context.Context, query *RoleQuery, nodes []*User, init func(*User), assign func(*User, *Role)) error {
	...
	query.Where(func(s *sql.Selector) {
		...
		joinT.Schema(uq.schemaConfig.UserRole)
...
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithNamedUserRole(name string, opts ...func(*UserRoleQuery)) *UserQuery {
	query := (&UserRoleClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if uq.withNamedUserRole == nil {
		uq.withNamedUserRole = make(map[string]*UserRoleQuery)
	}
	uq.withNamedUserRole[name] = query
	return uq
}
