...
// CreateTokenPair returns signed access and refresh tokens for the specified claims in one step
...
// CreateAccessToken from the credential payload or from an previous token if the access token is being reauthorized from previous credentials
...
// keyFunc selects the RSA public key from the list of managed internal keys based on the kid in the token header - if the kid does not exist an error is returned and the token will not be able to be verified
