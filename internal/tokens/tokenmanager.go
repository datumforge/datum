...
const DefaultRefreshAudience = "https://auth.datum.net/refresh"
...
func New(conf config.TokenConfig) (tm *TokenManager, err error) {
	...
	for kid, path := range conf.Keys {
		...
		// fmt.Errorf("could not parse kid %q for path %s: %s", kid, path, err)
		...
		// fmt.Errorf("could not read kid %s from %s: %s", kid, path, err)
		...
		// fmt.Errorf("could not parse RSA private key kid %s from %s: %s", kid, path, err)
...
// ParseUnverifiedTokenClaims from an access token
